...
const CommissionLogEntries: React.FC<{ commissionLogId: string, onClose: ()=> void }> = ({ commissionLogId, onClose}) => {
  ...
  const [totalCalculatedDeposited, setTotalCalculatedDeposited] = useState(0);
  ...
  useEffect(() => {
    ...
    const getDepositedValues = async () => { 
      ...
      setTotalCalculatedDeposited(commissionLog?.attributes?.totalCalculatedDeposited);
      setTotalCommissionDeposited(commissionLog?.attributes?.totalCommissionDeposited);
    };
    callAction();
    getDepositedValues();
  }, []);
  ...
  }, [page, pageSize, commissionLogId, selectedCarrier, startDate, endDate]);
  ...
  console.log(totalCalculatedDeposited)
  ...
  return (
    <>
      <div className='flex flex-row justify-between items-center'>
        <LoadingButtonNP
          onClick={onClose}
          className="w-[12rem] rounded-lg bg-body/80 mt-4 p-2 text-white  hover:bg-opacity-90 flex justify-center items-center"
      ...
      />
      <div>
        <p>Total Commission Deposited: $<strong>{totalCommissionDeposited}</strong> vs Total Calculated Deposited: $<strong>{Math.round(totalCalculatedDeposited * 100) / 100 }</strong></p>
      </div>
      {/* <LoadingButtonNP
        onClick={onClose}
