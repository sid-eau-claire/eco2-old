              ...
              <TableBody>
                {categoryProducts.map((product: any) => (
                  <TableRow key={product.id}>
                    ...
                    <TableCell>{(parseFloat(product.FYC) * 100).toFixed(2)}%</TableCell>
                    {isEditable && (
                      <TableCell>
...
const ProductForm = ({ product, productCategories, onSave, carrierId }: { product: any, productCategories: any, onSave: any, carrierId: string }) => {
  ...
  const handleChange = (e: any) => {
    ...
    if (name === 'FYC') {
      ...
      setFormData((prev: any) => ({ ...prev, [name]: (parseFloat(value) / 100).toString() }));
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Input
        name="FYC"
        type="number"
        value={formData.FYC ? (parseFloat(formData.FYC) * 100).toFixed(2) : ''}
        onChange={handleChange}
        placeholder="FYC (%)"
      />
      <Input
