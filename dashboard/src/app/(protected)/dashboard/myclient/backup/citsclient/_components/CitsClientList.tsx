...
import { getCitsClient  } from '../_actions/contact';
...
const ClientList = ({profileId}: { profileId: string }) => {
  const router = useRouter()
  const pathname = usePathname()
  const [showAddContact, setShowAddContact] = React.useState(false)
  const [citsClients, setCitsClients] = React.useState([])

  useEffect(() => {
    const fetchClients = async () => {
      const response: any = await getCitsClient(profileId);
      setCitsClients(response)
    };
    fetchClients();
  }, [])  
  ...
  const columns = React.useMemo(() => [
    ...
    {
      id: 'telephoneNumber',
      accessorFn: (row: any) => `${row.phone_country_code} ${row.area_code} ${row.dial_number}`,
  ...
  // console.log('citsClients', citsClients);

  return (
    <div className='relative'>
      <div className="flex flex-col items-end md:flex-row md:justify-end md:items-center mb-5">
      ...
      <div className="flex justify-between items-center bg-transparent font-medium mt-4 dark:bg-boxdark dark:text-white">
        <div>
          <p>You have {citsClients.length} clients.</p>
        </div>
