...
export default function PersonDetails({ client, provinces, onUpdateSuccess }: PersonDetailsProps) {
  ...
  const { control, handleSubmit, setValue, watch, formState: { errors } } = useForm<Client>({
    defaultValues: {
      ...client,
  ...
  const onSubmit = async (data: Client) => {
    const formattedData = {
      ...
      netWorth: data.netWorth === null || isNaN(Number(data.netWorth)) ? null : Number(data.netWorth),
  ...
  const renderField = (name: string, label: string, icon: React.ElementType, tooltip: string, type: string = "text", isRequired: boolean = false) => {
    return (
      <div className="space-y-2">
        <LabelWithTooltip icon={icon} label={label} tooltip={tooltip} />
        <Controller
          name={name as keyof Client}
          control={control}
        ...
        />
        {get(errors, name) && (
          <p className="text-red-500">{get(errors, `${name}.message`)}</p>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {renderField("firstName", "First Name", User, "Enter the client's first name", "text", true)}
        {renderField("lastName", "Last Name", User, "Enter the client's last name", "text", true)}
        ...
        {renderField("mobilePhone", "Mobile Phone", Phone, "Enter the client's mobile phone number")}
          ...
          <Controller
            ...
            render={({ field }) => 
              isEditing ? 
                ...
                <Input value={client.address?.address || ''} readOnly />
            }
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Controller
            ...
            render={({ field }) => 
              isEditing ? 
                <Input {...field} value={field.value || ''} /> : 
                <Input value={client.address?.postalCode || ''} readOnly />
            }
          />
        </div>
