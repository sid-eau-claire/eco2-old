...
export const updateOpportunity = async (id: string, opportunityData: {
  ...
  // Step 2: Make the PUT request to update the opportunity
  const response = await strapiFetch({
    method: 'PUT',
  ...
  if (response.status !== 200) { // 200 is the HTTP status code for OK
    throw new Error(`Error updating opportunity: ${response.errorMessage || response.statusText}`);
  }
