...
const InsuranceProduct =   ({ form, currentStep ,setCurrentStep, markComplete, isEditable }: { form: any, currentStep: number, setCurrentStep: any, markComplete: any, isEditable?: boolean }) => {
  ...
  useEffect(() => {
    fields.forEach((field, index) => {
      ...
      if (riderWithUL && product.productCategory == universalLifeInsuranceId) {
        ...
        const annualPremiumValue = monthlyBillingPremium * 12 - totalPremiumWOUL;
        updateRevenue(index, annualPremiumValue );
      } else {
        updateRevenue(index, product.annualPremium);
      }
    });
  }, [riderWithUL, fields.map(field => field.id)]); 
  ...
  useEffect(() => {
    const ulSelected = productCategoryWatchFields.some(category => (category == universalLifeInsuranceId && category != ''));
  ...
  return (
    <ColContainer cols="1:1:1:1" className=''>
      {/* <p>Beneva</p> */}
                ...
                {formData.appInsProducts[field.fieldIndex] && formData.appInsProducts[field.fieldIndex].productId != '' && (
                  <div className='col-span-2 grid grid-cols-3 gap-x-4'>
                      ...
                      <CInput
                        ...
                        step={0.01}
                        isEditable={isEditable}
                        // disabled={`appInsProducts[${field.fieldIndex}].productCategory` == 'Universal Life Insurance' ? false: true}
                      />
                      <CInput
                          label="Est. Revenue"
                          ...
                          form={form}
                      />
                      {formData.appInsProducts[field.fieldIndex].productCategory == universalLifeInsuranceId ? (
                        <div className='col-span-3 grid grid-cols-2 space-x-4'>
                          <CInput
                            label="Monthly Billing Premium"
      ...
      {riderWithUL ? (
        <div className='flex flex-row justify-end items-center space-x-8 mt-4 p-4'>
          <Select
        ...
        <div className='flex flex-row justify-end items-center mt-[1rem] space-x-4'>
          ...
          <CInput
            ...
            type="currency"
            required={false}
            className='max-w-[10rem] mb-0 bg-black'
            form={form}
            isEditable={false}
            disabled
          />        
