...
const InsuranceProduct = ({ form, currentStep, setCurrentStep, markComplete, isEditable }: { form: any, currentStep: number, setCurrentStep: any, markComplete: any, isEditable?: boolean }) => {
  ...
  const [riderWithUL, setRiderWithUL] = React.useState(false);
  const firstAccessRef = useRef(false);
  const completionRef = useRef(false);
  const [universalLifeInsuranceId, setUniversalLifeInsuranceId] = React.useState<number>(0);
  ...
  useEffect(() => {
    const fetchProducts = async (carrierId: number) => {
      ...
      const convertedProducts = response.reduce((acc: any[], product: any) => {
        ...
        if (name === "Universal Life Insurance") {
          setUniversalLifeInsuranceId(id);
        }
  ...
  const getTotalPremiumWOUL = useCallback(() => {
    return appInsProducts.reduce((acc: number, product: any) => {
      if (product.productCategory != universalLifeInsuranceId) {
        return acc + parseFloat(product.annualPremium || 0);
      }
      return acc;
    }, 0);
  }, [appInsProducts, universalLifeInsuranceId]);
  ...
  const updateRevenue = useCallback((index: number) => {
    ...
    if (riderWithUL && product.productCategory == universalLifeInsuranceId) {
      const totalPremiumWOUL = getTotalPremiumWOUL();
      const monthlyBillingPremium = Number(getValues(`monthlyBillingPremium`)) || 0;
      premium = monthlyBillingPremium * 12 - totalPremiumWOUL;
    }
  ...
  }, [getValues, products, corporateMarginPercentDefault, setValue, riderWithUL, universalLifeInsuranceId, getTotalPremiumWOUL]);
  ...
  useEffect(() => {
    updateAllRevenues();
  }, [appInsProducts, updateAllRevenues, riderWithUL]);
  ...
  useEffect(() => {
    const ulSelected = appInsProducts?.some((product:any) => product.productCategory == universalLifeInsuranceId);
    if ((ulSelected && !riderInteractionRef.current) || (!ulSelected && riderInteractionRef.current)) {
      setShowAskRiderForUL(ulSelected);
      riderInteractionRef.current = false;
    }
    if (!ulSelected) {
      setRiderWithUL(false);
    }
  }, [appInsProducts, universalLifeInsuranceId]);
  ...
  return (
    <Form {...form}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {field.productId && (
                      <>
                        ...
                        />
                        {field.productCategory == universalLifeInsuranceId && (
                          <>
                            <FormField
                              control={control}
                              name="monthlyBillingPremium"
                              render={({ field: monthlyField }) => (
                                <FormItem>
                                  <FormLabel>Monthly Billing Premium</FormLabel>
                                  <FormControl>
                                    <CurrencyInput 
                                      {...monthlyField} 
                                      disabled={!isEditable} 
                                      onChange={(value) => { 
                                        monthlyField.onChange(value); 
                                        updateAllRevenues();
                                      }} 
                                    />
                                  </FormControl>
      ...
      {showAskRiderForUL && isEditable && (
        <Dialog open={showAskRiderForUL} onOpenChange={setShowAskRiderForUL}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Universal Life Insurance Rider</DialogTitle>
            </DialogHeader>
            <p>Will you include rider(s) in this Universal Life Insurance case?</p>
            <DialogFooter>
              <Button onClick={handleConfirmRider} className="bg-primary">Yes</Button>
              <Button onClick={handleCancelRider} className="bg-primary">No</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
